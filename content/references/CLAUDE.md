...
Content baseline: 7  
Purpose: Govern Claude's work on this project's own frontend and backend artifacts, whichever project this file has been installed into - resolved via the active profile (Section 2), never assumed.
...
1. The agent cannot determine the answer or cannot determine how to proceed after bounded, approved inspection.
2. The agent finds two materially correct answers and no governing requirement, existing convention, or evidence selects between them.
3. The agent would have to answer a question itself in a way that changes or expands the approved scope.
...
- for case 1, include a conservative `A` option that keeps the current approved scope and a separate option for each valid next step the agent can name;
