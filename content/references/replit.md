...
This file, and this project's own `ai/**`/`.agents/skills/**`, can drift out of date against the engine's canonical versions - or be missing entirely on a brand-new project. Call `opsgate_sync_instructions` to check: it returns a manifest (`files`: `{path, size}`, no content) of every current file (this one, every `ai/*.md` instruction object, every skill workflow) - skill files install under `.agents/skills/`, a different directory name than this engine's own `replit-skills/` source. Content is deliberately not included in that call - fetch it one file at a time with `opsgate_sync_file(path)` for each manifest entry, never all at once. Write each file's returned content to its `path`, creating anything missing and overwriting anything that differs, then re-read each to confirm the write succeeded before continuing. Neither tool creates tenants or tokens - that stays a separate, deliberate step outside this file's scope. See `ai/maintenance.md` for the full instruction-maintenance workflow.
