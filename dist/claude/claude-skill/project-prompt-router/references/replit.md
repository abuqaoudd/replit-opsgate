...
3. Work through `PROJECT_SETUP.md`'s fields yourself: ask the user about each one in your own words, as a normal conversation, then write their answer into the file in place of that field's `[FILL IN: ...]` text, keeping its `<!-- key: ... -->` marker exactly as it is. Do not invent business facts, roles, or rules on the user's behalf; if they don't know an answer yet, leave that field's `[FILL IN: ...]` text in place and move on rather than guessing - it can be answered later and setup re-run.
