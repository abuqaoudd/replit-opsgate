...
Use this template only when the work can be implemented, verified, and rolled back as one bounded batch. If the work meets any complexity trigger in `METCO_REPLIT_PHASED_IMPLEMENTATION_TEMPLATE.md`, stop and switch to that template, generating only the next authorized phase prompt.

## Load first

Read `replit.md` and `ai/metco.md`. Automatically select the most specific internal mode and installed scenario skill from this outcome; do not ask the user to choose either. Then read `[only relevant ai files]`, the selected skill, and `ai/testing.md` for implementation or verification. Stop if a capability gate or required scope is not authorized.
...
| Never access | `metco-api/**`, `pipeline/**`, resolved aliases |
