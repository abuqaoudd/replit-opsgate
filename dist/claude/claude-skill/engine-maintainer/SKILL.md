...
3. Run `python3 tools/validate-engine.py` - it must report 0 warnings. Run `python3 tools/test-all.py` - it must report every check passing. Both must pass before the change is considered done; fix regressions, don't rationalize them.
