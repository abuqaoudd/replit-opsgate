...
- Import `tools/opsgate_tools.py`'s functions directly and call them in-process - the same pattern `opsgate_mcp_server.py` itself uses - rather than shelling out to a subprocess per call.
