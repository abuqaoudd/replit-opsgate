...
def read_json(relative_path):
    if relative_path in metco_contracts.CONTRACTS:
        return copy.deepcopy(metco_contracts.CONTRACTS[relative_path])
...
def fixture_data(path):
    normalized = str(path)
    for fixture in [*metco_fixtures.ROUTING_FIXTURES, *metco_fixtures.HITL_FIXTURES]:
    ...
    if normalized in ["state:ready-phased-state", "ready-phased-state"] or normalized.endswith("fixtures/state/ready-phased-state.json"):
        return copy.deepcopy(metco_fixtures.READY_PHASED_STATE)
    if normalized in ["reports:parsed-sample-report", "parsed-sample-report"] or normalized.endswith("fixtures/reports/parsed-sample-report.json"):
        return copy.deepcopy(metco_fixtures.PARSED_SAMPLE_REPORT)
    if normalized in ["gold:bounded-frontend-request", "bounded-frontend-request"] or normalized.endswith("canonical/examples/gold-standard/bounded-frontend-request.json"):
        return copy.deepcopy(metco_fixtures.GOLD_STANDARD_BOUNDED_FRONTEND_REQUEST)
    if normalized in ["gold:phased-migration-request", "phased-migration-request"] or normalized.endswith("canonical/examples/gold-standard/phased-migration-request.json"):
        return copy.deepcopy(metco_fixtures.GOLD_STANDARD_PHASED_MIGRATION_REQUEST)
    return None
...
def compile_replit_prompt(request, route):
    ...
    scope = request.get("scope") or {}
    return f"""# METCO {request.get("module") or "Replit Task"}

Deliver: **{request.get("outcome")}**

Automatically route this task through the installed METCO instructions. Selected routing evidence:
...
|---|---|---|---|
| TaskControlInstruction | every METCO task | outcome, acceptance, scope, selected mode/skill | does not override replit.md |
...
def compile_artifact_prompt(request, route):
    title = f"# METCO {route.get('deliverable', '').replace('_', ' ')}"
    routing = f"""Routing:
...
def cmd_lint_prompt(argv):
    ...
    # while the canonical prose templates (METCO_REPLIT_TASK_TEMPLATE.md,
    # METCO_REPLIT_PHASED_IMPLEMENTATION_TEMPLATE.md) and hand/agent-authored prompts that
...
def cmd_init_run(argv):
    ...
    write_python_data(run_dir / "request.py", "REQUEST", request, "# METCO run request")
    write_python_data(run_dir / "route.py", "ROUTE", route, "# METCO run route")
    write_python_data(run_dir / "gate_result.py", "GATE_RESULT", {"request_id": run_id, "status": "blocked" if route.get("blocked") else "pending_preflight", "failed_gates": ["capability_gate"] if route.get("blocked") else [], "missing_authority": route.get("missing_authority") or []}, "# METCO run gate result")
    write_python_data(run_dir / "handoff.py", "HANDOFF", {"request_id": run_id, "completed": False, "next_phase_ready": False, "blockers": route.get("missing_authority") or [] if route.get("blocked") else []}, "# METCO run handoff")
...
def cmd_release_notes(argv):
    old_root = argv[0] if argv else "../audit_unpack/metco-gpt-project-kit"
    ...
    print(f"""# METCO Kit Release Notes
...
def cmd_build_distributions(argv):
    ...
    shutil.copy2(claude_root / "metco-replit-task-builder.zip", claude_root / "skill.zip")
    ...
    write_python_data(ROOT_DIR / "dist/release_hashes.py", "RELEASE_HASHES", manifest, "# Generated METCO release hashes")
...
def cmd_build_replit_install(argv):
    ...
    write_text(target / "INSTALL.md", """# METCO Replit Install
...
def cmd_audit_engine(argv):
    ...
    check("root replit aligns with generated install", same_bytes(project_root / "replit.md", ROOT_DIR / "dist/replit-install/replit.md"), "replit.md compared to metco-kit/dist/replit-install/replit.md")
    ...
    for file_path in list_files(ROOT_DIR / "dist/replit-install/.agents/skills", lambda item: str(item).endswith("SKILL.md")):
        rel = file_path.relative_to(ROOT_DIR / "dist/replit-install")
        check(f"root {rel} aligns", same_bytes(project_root / rel, file_path), str(rel))
    check("Python contracts exist", (ROOT_DIR / "tools/metco_contracts.py").exists(), "metco-kit/tools/metco_contracts.py")
    check("tools exist", (ROOT_DIR / "tools/preflight.py").exists(), "metco-kit/tools/preflight.py")
...
def cmd_validate_kit(argv):
    ...
    assert_exists("tools/metco_contracts.py")
    for contract_name, contract_value in metco_contracts.CONTRACTS.items():
    ...
    for fixture in metco_fixtures.ROUTING_FIXTURES:
    ...
    for fixture in metco_fixtures.HITL_FIXTURES:
        data = fixture["data"]
    ...
    for required in ["METCO Vendor approvals", "metco-frontend-development", "Acceptance Criteria", "PASSED/FAILED/NOT RUN", "Mandatory HITL Gate", "HITL decision required", "HITL Gate Result", "Per-Action Gate"]:
    ...
    diff = json.loads(capture_python("diff-upgrade", ["../audit_unpack/metco-gpt-project-kit", "canonical"]))
...
def main():
    ...
    if command == "metco_tools":
        if len(sys.argv) < 2:
            usage(f"Usage: python3 tools/metco_tools.py <{'|'.join(sorted(COMMANDS))}> [args...]")
        command = sys.argv[1]
        args = sys.argv[2:]
    else:
        args = sys.argv[1:]
